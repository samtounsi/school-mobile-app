...
class ChatScreen extends StatefulWidget {
  ...
  @override
  State<ChatScreen> createState() => _ChatScreenState(chatModel: this.chatModel);
}
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void connect()
  {
    socket=IO.io('https://new-school-management-system.onrender.com',
        <String,dynamic>
        {
  ...
  {
    MessageModel messageModel=MessageModel(type, message,DateTime.now().toString().substring(10,16));
    setState(() {
      setState(() {
            ...
            appBar: AppBar(
              ...
              title: InkWell(
                onTap: ()
                {
                  ...
                  {
                    print(chatModel.id);
                    ChatCubit.get(context).getStudentProfile(id: chatModel.id!, year: (DateTime.now().month > 6)
                        ? DateTime.now().year + 1
                        : DateTime.now().year);
                ...
                child: Text(chatModel.fullName.toString(),
                  style: TextStyle(
                      color:AppColors.darkBlue,
                      fontSize: 25,
            ...
            body:Container(
              ...
              child: WillPopScope(
                ...
                child: ConditionalBuilder(
                  condition: state is ! ChatMessagesLoadingState&&ChatCubit.get(context).messagesHistoryModel!=null,
                  builder:(context)
                  {
                    model=ChatCubit.get(context).messagesHistoryModel?.messages;
                    return Column(
                      children: [
                        Expanded(
                                    ...
                                    itemBuilder: (context,index)
                                    {
                                      if(model.length!=0)
                                      {
                                        if(model[index].type=='sent')
                                        {
                                          ...
                                          if(type=='teacher')
                                          {
                                            return OnMessageCard(message: model[index]!.message
                                                ,model: TeacherCubit.get(context).teacherProfileModel,
                                                time:model[index]!.time.toString().substring(11,16));
                                          }
                                          if(type=='student')
                                          {
                                            return OnMessageCard(message: model[index]!.message
                                          ...
                                          {
                                            return OnMessageCard(message: model[index]!.message
                                                ,model: ParentCubit.get(context).parentProfileModel,
                                                time:model[index]!.time.toString().substring(11,16));
                                          }
                                        }
                                        ...
                                        {
                                          return ReplyCard(message: model[index]!.message,model: chatModel,time: model[index]!.time.toString().substring(11,16));
                                        }
                                      }
                                ...
                                ListView.builder(
                                  ...
                                  itemBuilder: (BuildContext context, int index)
                                  {
                                    ...
                                    if(messages[index]?.type=="source")
                                    {
                                      if(type=='teacher')
                                      {
                                        return OnMessageCard(message: messages[index]!.message
                                            ,model: TeacherCubit.get(context).teacherProfileModel,
                                    ...
                                    {
                                      return ReplyCard(message: messages[index]!.message,model: chatModel,time: messages[index]!.time);
                                    }
                                  },
                                  ...
                                  suffixpressed: ()
                                  {
                                    if(sendButton)
                                    {
                                      scrollController.animateTo(
                                          ...
                                          duration: Duration(milliseconds: 300),
                                          curve: Curves.easeOut);
                                      sendMessage(ChatCubit.get(context).messageController.text
                                          , id!, chatModel.userId!);
                  ...
                  fallback: (conetxt)=>Center(child: CircularProgressIndicator()),
                ),
              ),
            ),
          ),
