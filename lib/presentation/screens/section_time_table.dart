...
class SectionTimeTable extends StatelessWidget {
  const SectionTimeTable({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocConsumer<StudentTimetableCubit, StudentTimetableStates>(
      listener: (context, state) {
        if (state is GetSectionTimetableSuccessState) {
          model = state.studentTimetable.daysLessons;
        }
        if (state is GetSectionTimetableEmptyState) {
          model = state.empty.daysLessons;
        }
      },
      builder: (context, state) {
        model = StudentTimetableCubit.get(context).sectionTimetable == null
            ? StudentTimetableCubit.get(context).emptyTable!.daysLessons
            : StudentTimetableCubit.get(context).sectionTimetable!.daysLessons;
        return Scaffold(
            ...
            child: Container(
              ...
              child: Padding(
                ...
                child: ConditionalBuilder(
                  condition:
                      state != GetSectionTimetableLoadingState && model != null,
                  builder: (context) {
                    model =
                        StudentTimetableCubit.get(context).sectionTimetable ==
                                null
                    ...
                    return Column(
                      ...
                      children: [
                        ...
                        DataTable(
                          ...
                          columnSpacing:
                              state is GetSectionTimetableSuccessState
                                  ? 25
                                  : 35,
                  ...
                  fallback: (context) => Padding(
                    padding:
                        const EdgeInsets.only(right: 70.0, left: 70, top: 400),
